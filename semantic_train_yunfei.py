...
class TrainANN():  
    ...
    def LSTM(self,plot = True, save_model= True,max_words = 10000,maxlen = 250):
        """
        LSTM model
        ...
        history = model.fit(x_train, y_train, epochs=8, batch_size=200, validation_data=(x_val, y_val))
        if plot:
            pass
            #report(history,title="LSTM")
        if save_model:
            model.save('LSTM.h5')
        ## best acc: .58; var_acc: .56
        ...
        history = model.fit(x_train, y_train, epochs=10, batch_size=200, validation_data=(x_val, y_val),
                            verbose=1)
    def GRU(self,plot = True, save_model= True,max_words = 10000,maxlen = 250):
        """
        GRU model
        :param plot: whether to show the plots
        :param save_model: whether to save the model
...
if __name__ == '__main__':
    ...
    df.embedding(max_words = 10000,full_query = True,maxlen = 250,glove_dir = '/Users/irene/Downloads/glove.6B')
    df.LSTM(max_words = 10000,maxlen = 250)
    df.GRU(max_words = 10000,maxlen = 250)